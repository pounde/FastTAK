...
class TestBuildEnrollmentUrl:
    def test_basic_url(self):
        url = build_enrollment_url(token="abc123", fqdn="tak.example.com", username="jdoe")
        assert (
            url == "tak://com.atakmap.app/enroll?host=tak.example.com&username=jdoe&token=abc123"
        )

    def test_includes_all_params(self):
        url = build_enrollment_url(token="mytoken", fqdn="tak.example.com", username="epound")
    ...
    def test_special_characters_in_token(self):
        url = build_enrollment_url(
            token="tok+en/with=chars", fqdn="tak.example.com", username="user1"
        )
