...
class TestPoll:
    ...
    def test_calls_health_fn_and_updates_store(self):
        ...
        mock_eval.assert_called_once_with("database", raw, service_config["thresholds"])
        mock_store.update.assert_called_once()
        # Status ok < warning alert_min_level, so no alert
    ...
    def test_alerts_when_status_meets_min_level(self):
        ...
        with (
            ...
            patch(
                ...
                return_value={"status": "warning", "message": "size_bytes is high"},
            ),
            patch("app.scheduler.check_and_alert") as mock_alert,
        ):
            from app.scheduler import _poll
    ...
    def test_no_alert_below_min_level(self):
        ...
        with (
            patch("app.scheduler.store"),
            patch("app.scheduler.evaluate", return_value={"status": "ok"}),
            patch("app.scheduler.check_and_alert") as mock_alert,
        ):
            from app.scheduler import _poll
