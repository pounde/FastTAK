...
## DD-028: Health Monitoring Architecture Refactor
...
## DD-027: Live Data Size Uses Heap-Only Measurement
...
## DD-026: Global Autovacuum Tuning via Startup Flags

**Date:** 2026-03-27
**Status:** Decided

**Decision:** PostgreSQL autovacuum is tuned via global `-c` startup flags injected by `tak-database/start.sh`, not per-table `ALTER TABLE SET` statements. Three configurable `.env` variables with FastTAK defaults: `PG_AUTOVACUUM_SCALE_FACTOR` (default: 0.05), `PG_AUTOVACUUM_COST_LIMIT` (default: 1000), `PG_MAINTENANCE_WORK_MEM` (default: 256MB). These defaults are applied automatically — operators who want stock PostgreSQL behavior must explicitly set them to `0.2`, `200`, `64MB`. Autovacuum health status uses a minimum dead tuple threshold (`AUTOVACUUM_MIN_DEAD_TUPLES`, default 1000) — tables below this count are shown in the UI but don't affect the overall status. This prevents tiny config tables with a handful of dead rows from triggering false alerts on a fresh stack.
...
## DD-025: Monitor Becomes the API Service
...
## DD-024: User Cert Enrichment is Opt-In
...
## DD-023: User TTL via Revocation Scheduler
...
## DD-022: Service Account Naming Convention
...
## DD-021: Passwordless Users by Default

**Date:** 2026-03-25
**Status:** Decided

**Decision:** User creation does not set a password. Passwords are set explicitly via a separate operation if the user needs browser access (WebTAK, admin UI, Caddy-protected services). Device-only users never have an account password.
...
## DD-020: Enrollment Token TTL is Configurable
...
- See [DD-025](#dd-025-monitor-becomes-the-api-service) for how the monitor service was formalized as the API service

---

## DD-017: Docs Audience is Operators First

**Date:** 2026-03-24
**Status:** Decided

**Decision:** Documentation (mkdocs) targets operators deploying FastTAK — setup guides, config reference, troubleshooting. Contributor/developer docs are secondary.

**Why:** The primary audience right now is people standing up TAK infrastructure, not people extending the codebase.

---

## DD-016: Release Path Filtering is Exclude-Only

**Date:** 2026-03-24
**Status:** Decided

**Decision:** Semantic release triggers on any `feat:`/`fix:` commit unless the commit *only* touches `tests/`, `docs/`, or `.github/`. New directories and services trigger releases by default.
...
## DD-015: Semantic Release with 0.x Versioning

**Date:** 2026-03-24
**Status:** Decided

**Decision:** `allow_zero_version = true`, `major_on_zero = false`. Breaking changes bump minor (0.x.0), not major (1.0.0). Version tracked via `version_toml` in `pyproject.toml`.

**Why:** Project is pre-1.0. Jumping to 1.0.0 on a breaking change is premature.

---

## DD-014: CI Runs Unit Tests Only

**Date:** 2026-03-24
**Status:** Decided (constraint-driven)

**Decision:** GitHub Actions CI runs unit tests (`just test`) and shellcheck only. Full integration tests (`just test-integration`) stay manual/local. A placeholder exists for a future integration job.
...
## DD-013: Pre-push for Tests, Pre-commit for Linting

**Date:** 2026-03-24
**Status:** Decided

**Decision:** Pre-commit hooks run ruff (lint + format), standard hooks (trailing whitespace, YAML, merge conflicts), and shellcheck. Pytest runs on pre-push, not pre-commit.

**Why:** Full test suite on every commit is too slow. Pre-push is the right gate for tests.

---

## DD-012: Testing Uses uv and just, Separate from Production

**Date:** 2026-03-24
**Status:** Decided

**Decision:** `pyproject.toml` lives at the project root (not `monitor/`). Test dependencies are in a uv dev dependency group, completely separate from production `monitor/requirements.txt`. `just` is the task runner (not `make`).

**Why:** Testing is a project-level concern. `uv` keeps test deps out of the production Docker image. The user prefers `just` over `make`.

---

## DD-011: Tilde Pins at Patch Level for Python Dependencies
...
## DD-010: Loki Over PostgreSQL for Future Log Aggregation

**Date:** 2026-03-24
**Status:** Decided (direction, not yet implemented)

**Decision:** If log aggregation is ever needed, use Loki (label-indexed, cheap storage) rather than pushing container logs to PostgreSQL.

**Why:** PostgreSQL would double DB write load, compound unbounded growth, and add retention complexity. The 80/20 solution is correlating container log snippets at the moment of health transitions.

---

## DD-009: Modern Ciphers for Cert Generation
...
**Why:** "We tool for the modern times. If people are using 8-year-old software, it's not my problem."

---

## DD-008: Node-RED Subflows Over npm Packages

**Date:** 2026-03-23
**Status:** Decided

**Decision:** Reusable Node-RED components (GeoJSON-to-CoT, CoT-to-GeoJSON, Geofence Alert) are built as subflows baked into `flows.json`, not as `node-red-contrib-*` npm packages.

**Why:** Ships with the container, shows up in the palette, no npm package to maintain. Can extract to npm later if useful to the broader TAK community.

---

## DD-007: Node-RED flows.json Ships Config Nodes Only

**Date:** 2026-03-23
**Status:** Decided

**Decision:** `flows.json` contains only invisible config nodes (PostGIS connection, TLS config) and a comment. No pre-wired TCP/TAK nodes on the canvas.

**Why:** Users wire their own flows and select configs from dropdowns. Keeps the starter workspace clean.
...
**Date:** 2026-03-23
**Status:** Superseded by [DD-025](#dd-025-monitor-becomes-the-api-service)
...
**Superseded:** The monitor service was formalized as the FastTAK management API in [DD-025](#dd-025-monitor-becomes-the-api-service). It now holds the Authentik API token and communicates with TAK Server via mTLS for cert and user operations.
...
**Why:** CloudTAK is a 6-service stack on its own (API, tile server, event processor, MediaMTX, PostGIS, MinIO). It bundles its own MediaMTX (port conflict) and needs its own PostGIS + MinIO. It's fundamentally a client-side platform; FastTAK is server-side infrastructure.
...
**Decision:** The stack has no `--profile` flags. All services (TAK Server, Authentik, Caddy, MediaMTX, Node-RED, TAK Portal) are always present.

**Why:** Profiles added complexity without benefit — Authentik/LDAP/TAK Portal were being used for all testing anyway. A single stack is simpler to document, test, and support.
