...
documented as a step. `start.sh` and `reconfig.sh` pass `--remove-orphans` (as
`just up` already did); `init-identity` bootstraps **every** gate group
(`ADMIN_GROUP` and `BACKUP_ADMIN_GROUP`) and joins `webadmin` to each;
`scripts/check-env.sh` refuses to start on a missing or unsafe value. `docs/upgrading.md`
covers only what genuinely requires an operator decision — chiefly which accounts
get admin access.

**Why:** Two failures on the same deployment showed that upgrade rules kept as
prose don't get followed. `docker compose up` does not remove containers whose
service was deleted from the compose file, so `tak-portal` — removed in DD-043
precisely because it had become unauthenticated — kept running for months,
invisible to `docker compose ps`. Separately, DD-047 introduced `ADMIN_GROUP`
while the bootstrap knew only `BACKUP_ADMIN_GROUP`; renaming the former produced
a group nothing created and nobody joined, 403-ing every route on the monitor
including for `webadmin`, with no route back in through the UI.

The dividing line: if the correct action is always the same, it belongs in a
script. If it needs checking but no judgment, it belongs in the preflight. Only
genuine decisions belong in prose.

**Retroactive by construction:** `init-identity` runs on every stack start and
its `ensure_group`/`add_to_group` helpers are idempotent, so existing
deployments repair themselves on the next restart rather than needing a
migration.

**Alternatives considered:** A dedicated upgrade script — rejected, it would
...
## DD-047: The monitor is admin-only, end to end

**Decision:** Every route on the monitor requires membership in `ADMIN_GROUP`
(default `monitor_admin`) — the JSON APIs (`/api/users`, `/api/service-accounts`,
`/api/ops`, `/api/health`, `/api/events`, `/api/tak`) and every dashboard page
...
enough: `/ui/partials/user-list` calls the identity client directly and returned
the full user roster to any authenticated caller, and health telemetry (container
inventory, cert expiry, config drift) plus `/api/events` (the audit trail) are
operator data, not user data. Nothing on the monitor serves non-admin users —
enrollment reaches TAK users as a QR code or `tak://` link, generated by an admin
— so a blanket gate costs no legitimate access.

**Consequences:** Callers that reach the monitor without Caddy in front must
supply `Remote-User`/`Remote-Groups` themselves. The test stack publishes the
monitor port directly, so `tests-integration/conftest.py` sends admin headers on
every request; production does not publish the port.

**Limits of this gate (added after review).** It authorizes external callers; it
does not authenticate the header's origin. The monitor cannot tell a Caddy-proxied
request from one issued by any other container on the Docker network, so anything
with code execution there — notably Node-RED, whose editor every authenticated
LDAP user can reach — can present `Remote-Groups: monitor_admin` and be treated
as an admin. Not publishing the monitor's port is necessary but not sufficient.
Closing it needs a Caddy-injected shared secret the monitor requires, or network
isolation. Upgrading also has a membership prerequisite: accounts other than
`webadmin` must join the gate group before the upgrade, and `ADMIN_GROUP` must be
bootstrapped when renamed (see DD-048).

**Alternatives considered:** Gate only the mutating routes — rejected, it leaves
the roster and audit trail readable by any TAK user. Gate the dashboard router as
a whole — rejected, it would force `ADMIN_GROUP` onto the backup pages, so an
operator who points `BACKUP_ADMIN_GROUP` at a separate group would find their
backup admin locked out of the pages that group exists to grant.
